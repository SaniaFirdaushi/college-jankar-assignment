...
router.post('/send-verification', async (req, res) => {
  ...
  const token = jwt.sign({ email }, 'secretKey', { expiresIn: '1h' }); // Replace 'secretKey' with an env variable
  ...
  const mailOptions = {
    ...
    text: `Click the link to verify your email: http://yourwebsite.com/verify/${token}`,
  };
...
router.get('/verify/:token', async (req, res) => {
  try {
    const decoded = jwt.verify(req.params.token, 'secretKey'); // Replace 'secretKey' with an env variable
